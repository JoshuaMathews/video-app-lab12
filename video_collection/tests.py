...
class TestAddVideos(TestCase):
    ...
    def test_add_video_no_notes_video_added(self):
        ...
        # video list shows new video
        self.assertContains(response, valid_video['name'])
...
class TestVideoDetail(TestCase):
    ...
    def test_add_check_video(self):

        name = "example"
        url = "https://www.youtube.com/watch?v=67890"
        notes = "Example Notes"

        v1 = Video.objects.create(name=name, url=url, notes=notes)

        visit_place_url = reverse('video_info', args=(1,))
    ...
    def test_check_404_video(self):
        visit_place_url = reverse('video_info', args=(10654654654654654654,))
        response = self.client.post(visit_place_url, follow=True)
